...
class MatomoTracker {
    ...
    initialize({ urlBase, siteId, userId, trackerUrl, srcUrl, disabled = false, heartBeat, linkTracking = true, configurations = {}, }) {
        var _a;
        if (disabled) {
            // If disabled, make all tracking calls no-ops by not initializing _paq or the script
            this.pushInstruction = (..._args) => this; // No-op
            // Make other methods no-op as well
            const noOp = () => { };
            this.enableHeartBeatTimer = noOp;
            this.enableLinkTracking = noOp;
            this.trackEvent = noOp;
            this.trackPageView = noOp;
            this.trackSiteSearch = noOp;
            this.trackLink = noOp;
            this.trackEvents = noOp;
            this.stopObserving = noOp;
            this.addEcommerceItem = noOp;
            this.removeEcommerceItem = noOp;
            this.clearEcommerceCart = noOp;
            this.trackEcommerceOrder = noOp;
            this.trackEcommerceCartUpdate = noOp;
            this.setEcommerceView = noOp;
            this.setEcommerceCategoryView = noOp;
            this.requireConsent = noOp;
            this.setConsentGiven = noOp;
            this.requireCookieConsent = noOp;
            this.setCookieConsentGiven = noOp;
            this.forgetCookieConsentGiven = noOp;
            this.optUserOut = noOp;
            this.forgetUserOptOut = noOp;
            this.deleteCookies = noOp;
            return;
        }
    ...
    trackEvent(_a) {
        ...
        if (category && action) {
            this.track(Object.assign({ data: [constants_1.TRACK_TYPES.TRACK_EVENT, category, action, name, value] }, otherParams));
        }
        else {
    ...
    trackSiteSearch(_a) {
        ...
        if (keyword) {
            this.track(Object.assign({ data: [constants_1.TRACK_TYPES.TRACK_SEARCH, keyword, category, count] }, otherParams));
        }
        else {
    ...
    trackLink({ href, linkType = "link", customDimensions, }) {
        if (customDimensions &&
            Array.isArray(customDimensions) &&
            customDimensions.length) {
            customDimensions.forEach((customDimension) => this.pushInstruction("setCustomDimension", customDimension.id, customDimension.value));
        }
    ...
    trackPageView(params) {
        this.track(Object.assign({ data: [constants_1.TRACK_TYPES.TRACK_VIEW] }, params));
    }
    ...
    track({ data = [], documentTitle, // Changed: use passed documentTitle or fallback to window.document.title later
    href, customDimensions = false, }) {
        if (typeof window === "undefined")
            return;
        if (data.length) {
            if (customDimensions &&
